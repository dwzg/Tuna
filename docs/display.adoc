...
----
  segment.c  ──┐
               ├──► display_set_digit() ──► display.c shadow[] ──┐
  bargraph.c ──┘                                                 │
                                                                 ▼
  display_flush()  (diff shadow[] vs shipped[], send only changes)
                                                                 │
                                                                 ▼
  max7219_write() ──► HAL pins (PC1/PC2/PC3, bit-banged) ──► MAX7219
----
...
| `display_set_digit(digit, value)`
| Stage a raw segment byte into `shadow[digit]`. Out-of-range indices are
  ignored. Nothing is transmitted yet.

| `display_flush()`
| For each digit where `shadow[digit] != shipped[digit]`, write it to the
  matching `MAX7219_DIGIT_n` register and update `shipped[digit]`. Unchanged
  digits cost no bus traffic.
|===
...
| `segment_display_char(digit, alpha, dotpoint)`
| Render uppercase letter `alpha` on `digit` (0 or 1). If `dotpoint` is set, OR in
  the decimal-point segment (`0x80`). Used for note letters, with the dot marking
  accidentals.

| `segment_display_alpha(digit, alpha)`
| Convenience wrapper: a letter with no decimal point.

| `segment_display_num_digit(digit, value, dotpoint)`
| Render decimal digit `value` (0–9) on `digit`, optional decimal point. Used for
  the octave number.

| `segment_smile()`
| Stage the two-digit smiley (splash only).
|===

All four *stage* into the framebuffer; a `display_flush()` is needed to show them.
...
| `bargraph_set_level(level, origin)`
| Light a contiguous run of `level` LEDs from either end. `origin` is
  `BARGRAPH_LEFT` (run grows from the left) or `BARGRAPH_RIGHT` (run sits against
  the right). Used by the splash's growing bar.

| `bargraph_set_binary(value)`
| Set the LED pattern directly from a raw bitmask. Used for the tuning needle,
  where a single bit is lit at the cents position.
|===
...
* *Valid note:*
** *Digit 0* — the note letter via `segment_display_char()`. The accidental flag
   (`NOTE_ACCIDENTAL[]`) lights the decimal point for sharps/flats. The letter
   tables match the chosen accidental convention (German `H` for B).
** *Digit 1* — the octave number via `segment_display_num_digit()`.
** *Bar graph* — the cents deviation as a centred needle. The ±50 cents range is
   mapped onto LED positions `0..BARGRAPH_SIZE-1`:
+
[source,c]
----
...
. `HI` with a small left-origin bar (level 6), 1 s.
. The `:)` smiley with a longer bar (level 13), 1 s.
. `TU` (level 16), 0.5 s.
. `NA` (level 20, full bar), 0.5 s.
. Clear digits and bar, ready for tuning.
...
| `max7219_init()`
| Idle the pins (`DIN` low, `CLK` low, `LOAD` high), `max7219_reset()`, then set
  scan limit to 4 (digits 0–4), intensity to max (`0x07`), and leave shutdown
  (normal operation).

| `max7219_write(address, data)`
| Clock a 16-bit frame (address in the high byte, data in the low byte) out
  MSB-first: `LOAD` low, then for each bit set `DIN` and pulse `CLK` high/low,
  then `LOAD` high to latch.

| `max7219_reset()`
| Write `0x00` to every register `0x00..0x0F`, defining the known-zero state the
  shadow framebuffer's `shipped[]` assumes.
|===
