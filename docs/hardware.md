...
> shift clock to **PC2**. On the AVR64DD14 there is no SPI0/USART pin-mux option
> that can output a hardware shift clock on PC2 while simultaneously driving serial
> data on PC1. A hardware serial peripheral therefore cannot drive this display
> layout, so the three lines are toggled in software in `max7219_write()`.
